...
/**
 * class used to test CouresRecordIO.
 * 
 * @author SarahHeckman
 * @author yueyu
 */
public class CourseRecordIOTest {
	...
	/** Tests the construction of a CourseRecordIO object */
	 ...
	 * 
	 * @throws Exception if there is a IOException
	 */
	@Before
	...
	public void testReadValidCourseRecords() throws InvalidTransitionException {
		try {
			...
			for (int i = 0; i < validCourses.length; i++) {
				assertNotEquals(validCourses[i].length(), courses.get(i).toString());
			}
